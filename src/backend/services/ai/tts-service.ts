...
/**
 * Generate TTS audio with per-scene duration tracking for synchronization.
 * Each scene's narration is generated individually so we can measure its duration
 * and pad the video timeline accordingly.
 */
export async function generatePerSceneNarration(
  narrations: string[],
  jobId: string,
  onProgress?: (scene: number, total: number) => void,
): Promise<PerSceneTtsResult | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY environment variable is not set.');
  }

  const hasAny = narrations.some(n => n && n.trim().length > 0);
  ...
  for (let i = 0; i < narrations.length; i++) {
    ...
    try {
      ...
      console.log(`[TTS] Scene ${i + 1}: ${dur.toFixed(1)}s (${(pcmBuffer.length / 1024).toFixed(0)} KB)`);
    } catch (err: any) {
...
export function mergeAudioWithVideo(
  ...
  // -af "apad" extends audio with silence to match video length
  // -shortest then truncates to video duration (the longer stream after padding)
  // This ensures: audio is never cut mid-sentence (video was pre-padded to be >= audio)
  // and the final output matches the video duration exactly
  const cmd = [
    `"${FFMPEG}"`, '-y',
    '-i', `"${videoPath}"`,
    '-i', `"${audioPath}"`,
